...
// Ruta de Inicio (Con filtro de activos)
...
// Ruta del Catálogo de Servicios (Única y limpia)
...
// RUTA PÚBLICA PARA EL CATÁLOGO DE INDUMENTARIA (Vistas del portal)
app.get('/indumentaria', async (req, res) => {
    try {
        // Opcional: Si en un futuro gustas listar la indumentaria de la BD acá
        // const [indumentaria] = await db.query('SELECT * FROM indumentaria');

        res.render('portal/indumentaria', { 
            title: 'Indumentaria y Merch | Búho Barber Studio'
            // indumentaria
        });
    } catch (error) {
        console.error('Error al cargar la página de indumentaria:', error);
        res.status(500).send('Error al cargar la página de indumentaria');
    }
});
